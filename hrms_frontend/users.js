...
async function loadUsers() {
  try {
    ...
    // Extra client-side safety: exclude Admin and HR Admin from HR portal view
    const ADMIN_ROLES = ['Admin', 'HR Admin'];
    allUsers = (Array.isArray(data) ? data : (data.users || data)).filter(
...
closeAddForm();
await loadUsers();
applyView(); // immediately refresh cards
  } catch (err) {
    showToast(err.message, 'error');
  } finally {
    btn.disabled = false;
...
function openDocsModal(userId, userName) {
  ...
  /* safely reset upload zone elements (removed in HR view) */
  const fInput   = document.getElementById('docFileInput');
  const fName    = document.getElementById('chosenFileName');
  const uploadBt = document.getElementById('docUploadBtn');
  if (fInput)   fInput.value = '';
  if (fName)    fName.style.display   = 'none';
  if (uploadBt) uploadBt.style.display = 'none';
  loadDocs();
}
