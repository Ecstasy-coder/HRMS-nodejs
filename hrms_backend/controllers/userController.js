...
exports.getUsers = async (req, res) => {
  try {
    ...
    // Hide Admin and HR Admin from HR Portal Manage Users page
    // Only show Employees, Reporting Managers, Finance, and other non-admin roles
    const users = await User.find({
      role: { $nin: ['HR Admin', 'Admin'] }
    })
      .select('-password')
      .sort({ createdAt: -1 });
